...
class CseDriveClient(object):
  ...
  def generate_cse_token(self, parent_id=None, file_id=None):
    """Generate a CSE Token.

    Args:
        parent_id: Generate a token for a new file in this parent in Drive. If
          None, use the root MyDrive of `delegated_user_email`.
        file_id: Generate a token this existing file. If None, Drive will
          auto-generate a new id.

    - Omit file_id when generating a token to upload a new file.
    - Use file_id when generating a token to update (upload a new revision) an
      existing file.
    It is invalid to specify both parent_id and file_id.

    Returns:
        A CSE Token, as a dict.
    """
    if parent_id is not None and file_id is not None:
      raise ValueError('Cannot set both parent_id and file_id')
    with self._service() as service:
      request = service.files().generateCseToken(
          parent=parent_id, fileId=file_id
      )
  ...
  def cse_update(
    ...
    """Upload the given data as a new revision of an existing CSE file.

    Args:
      data_fp: The ciphertext data to upload; a readable file-like object.
      file_id: The Drive id of the file being updated.
      filename: The name of the file being updated.
      encryption_details: Encryption metadata for the file being updated.
      content_type: The content-type of the file being updated.

    Returns:
      The metadata of the newly updated file, as a dict.
    """
    file_metadata = {
        'name': filename,
        'filename': filename,
